...
import { Loader2, ChevronRight, FileText, ArrowLeft, Menu, Clock, ChevronUp, PanelLeftClose, PanelLeftOpen } from 'lucide-react'
...
function StudentProgramView() {
  ...
  const [sidebarHidden, setSidebarHidden] = useState(false) // desktop hide toggle
  const [showScrollTop, setShowScrollTop] = useState(false)
  const progressRef = useRef(null)

  // Reading progress — direct DOM manipulation (no re-render!)
  useEffect(() => {
    const onScroll = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('samyak_sidebar_hidden')
    if (saved === '1') setSidebarHidden(true)
  }, [])
  const toggleSidebar = () => {
    setSidebarHidden(v => {
      const next = !v
      localStorage.setItem('samyak_sidebar_hidden', next ? '1' : '0')
      return next
    })
  }
    ...
    <>
      <div className="px-5 py-5 border-b border-slate-200/60">
        <div className="flex items-center justify-between mb-3">
          <Link href="/dashboard" className="text-xs text-slate-500 hover:text-slate-900 inline-flex items-center gap-1 transition"><ArrowLeft className="h-3 w-3" />All programs</Link>
          <button onClick={toggleSidebar} className="hidden md:inline-flex h-7 w-7 items-center justify-center rounded-md text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition" title="Hide sidebar">
  ...
  return (
    <div className="min-h-screen bg-canvas flex relative">
      <div ref={progressRef} className="reading-progress" style={{ width: '0%' }} />

      {/* Desktop sidebar with slide animation */}
      <motion.aside
        animate={{ width: sidebarHidden ? 0 : 288, opacity: sidebarHidden ? 0 : 1 }}
        transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
        className="hidden md:flex bg-white border-r border-slate-200/60 flex-col sticky top-0 h-screen z-10 overflow-hidden"
      >
      ...
      {/* Floating "Show sidebar" button when hidden (desktop) */}
      {sidebarHidden && (
        <button
          onClick={toggleSidebar}
          className="hidden md:flex fixed top-4 left-4 z-50 h-9 w-9 items-center justify-center rounded-lg bg-white border border-slate-200 shadow-sm text-slate-700 hover:bg-slate-50 transition"
          title="Show sidebar"
        >
